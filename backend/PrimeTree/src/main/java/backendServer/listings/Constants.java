...
public class Constants {
	...
	public static class LocationsLoader{
		
		StandOrtRepository standortRepository;
		
		public LocationsLoader(StandOrtRepository standortRepository){
			this.standortRepository=standortRepository;
		}
		
		@Autowired
		public void setAllLocations(){
			Iterator<Location> standortIterator=standortRepository.findAll().iterator();
			Constants.allLocations=new LinkedList<String>();
			while(standortIterator.hasNext()){
				allLocations.add(standortIterator.next().getOrtName());
			}
		}
	}
